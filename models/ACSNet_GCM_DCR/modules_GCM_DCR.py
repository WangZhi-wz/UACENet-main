...
class GCM1(nn.Module):
    ...
    def forward(self, x, lca):
        ...
        out = global_context + residual +  lca
...
class GCM23(nn.Module):
    def __init__(self, in_channels, in_channel_lca, reduction=16, kernel_size=7):
        super(GCM23, self).__init__()
        pool_size = [1, 3, 5]
        out_channel_list = [256, 128, 64, 64]
        upsampe_scale = [2, 4, 8, 16]
        GClist = []
...
class GCM4(nn.Module):
    ...
    def forward(self, x):
        ...
        return global_context + residual
