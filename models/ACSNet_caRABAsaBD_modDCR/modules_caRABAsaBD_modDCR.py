...
class RA1(nn.Module):
    ...
    def forward(self, x, pred, lca):
        ...
        # RA (background)
        att_ra = torch.sigmoid(-pred) # background Weight
        att_ra = x * att_ra  # a与x逐元素乘法

        # LCA (edge)
        score = torch.sigmoid(pred)
        dist = torch.abs(score - 0.5)
        att_lca = 1 - (dist / 0.5) # edge Weight
        att_lca = x * att_lca
...
class FAAM(nn.Module):
    ...
    def forward(self, x, pred, lca):
        ...
        # RA (background)
        att_ra = torch.sigmoid(-pred) # background Weight
        att_ra = x * att_ra  # Reverse attention feature
        # LCA (edge)
        score = torch.sigmoid(pred)
        dist = torch.abs(score - 0.5)
        att_lca = 1 - (dist / 0.5) # edge Weight
        att_lca = x * att_lca # Boundary concern feature
