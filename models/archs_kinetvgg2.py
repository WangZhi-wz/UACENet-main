...
class asa_layer(nn.Module):
    ...
    def forward(self, x, y):
        ...
        # bilinear pooling
        W_g = self.W_g(x) # 1,64,192,192
        W_x = self.W_x(y) # 1,64,192,192
        bp = self.W(W_g * W_x) # 1,64,192,192
        bp = bp.reshape(b * self.groups, -1, h, w) # 16,4,192,192

        x = x.reshape(b * self.groups, -1, h, w) # 16,4,192,192
        y = y.reshape(b * self.groups, -1, h, w) # 16,4,192,192
        x_0 = x # 16,4,192,192
        x_1 = y # 16,4,192,192

        xn = self.avg_pool(x_0) # 16,4,1,1
        xn = self.cweight * xn + self.cbias # 16,4,1,1
        xn = x_0 * self.sigmoid(xn) # 16,4,192,192

        xs = self.gn(x_1) # 16,4,192,192
        xs = self.sweight * xs + self.sbias # 16,4,192,192
        xs = x_1 * self.sigmoid(xs) # 16,4,192,192

        # concatenate along channel axis
        out = torch.cat([xn, xs, bp], dim=1) #16,12,192,192
        out = out.reshape(b, -1, h, w) #1,192,192,192

        out = self.channel_shuffle(out, 3) #1,192,192,192
        out = self.oneconv1(out) #1,64,192,192
        return out
...
class DoubleDownsample(nn.Module):
    ...
    def forward(self, x):
        x1 = self.norm1(self.conv(x)) #1,64,48,48
        c1 = self.pool(x) #1,32,48,48
        c2 = self.avgpool(x) #1,32,48,48
        x2 = self.norm2(self.residual(torch.cat([c1,c2],1))) #1,64,48,48
        x = self.act(x1 + x2) #1,64,48,48
        return x
...
class dbunetinkifdatrnewfddnbakmmfalre30(nn.Module):
    ...
    def forward(self, x):
        # 1,3,96,96
        out = self.encoder1(x)  #1,32,96,96
        t0 = out #1,32,96,96
        out = self.encoder2(self.ds1(out)) #1,64,48,48
        t1 = out #1,64,48,48
