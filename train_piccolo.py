...
def test(model, test_dataloader, total_batch):
    ...
    with torch.no_grad():
        ...
        for i, data in bar:
            ...
            # output = model(img)
            output = model(img, flag = "test")  # 1,1,320,320/160/80/40/20
...
def train():
    ...
    #     log_dir="./weights/logs_" + opt.model + "_" + opt.expName + "_" + str(x.strftime('%Y_%m%d_%H%M%S')))
    ...
    for epoch in range(opt.nEpoch):
        ...
        # ---------------------------------------- Valid -------------------------------------------------
        metrics_result, val_loss_all = valid(model, valid_dataloader, val_total_batch)
        ...
        print('Dice: %.4f, IoU_mean: %.4f, precision: %.4f, ACC_overall: %.4f, IoU_poly: %.4f,'
              ...
              % (metrics_result['Dice'], metrics_result['IoU_mean'],
                 ...
                 metrics_result['F1'], metrics_result['F2'], val_loss_all))
